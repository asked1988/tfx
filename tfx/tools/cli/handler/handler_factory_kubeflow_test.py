...
"""Tests for tfx.tools.cli.handler.handler_factory."""
...
class HandlerFactoryKubeflowTest(tf.test.TestCase):

  def setUp(self):
    super(HandlerFactoryKubeflowTest, self).setUp()
    self.flags_dict = {}
  ...
  def test_detect_handler_missing(self):
    ...
    self.assertEqual(
        str(cm.exception), 'Orchestrator missing in the environment.')
